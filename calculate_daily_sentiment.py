...
def load_year_file(year: int, csv_dir: Path) -> pd.DataFrame | None:
    """Load oil_news_YYYY_result.csv for a given year."""
...
def calculate_daily_sentiment(
    ...
    """
    csv_dir = Path(csv_dir) if csv_dir else CSV_RESULTS_DIR
    output_path = Path(output_path) if output_path else OUTPUT_FILE

    dfs = []
    for year in range(start_year, end_year + 1):
        df = load_year_file(year, csv_dir)
        if df is None:
            continue
        df["year_loaded"] = year
        dfs.append(df)
    ...
    if not dfs:
        raise FileNotFoundError(
            f"No oil_news_YYYY_result.csv files found in {csv_dir} for years {start_year}-{end_year}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input", "-i", type=str, default=None,
                        help="Input dir: path or subdir under csv_results (e.g. 'cleaned')")
